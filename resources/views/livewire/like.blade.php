...
@if(Auth::check())
    ...
    @if ($isDisliked ?? false)
    <a wire:key="is2liked-{{$post->id}}" wire:click="undoDislike" title="undo dislike" class="ml-2 cursor-pointer">
        <x-microblog.images.unlike-icon class="fill-red-500 hover:stroke-cyan-700"/>
    </a>
    @elseif($isLiked ?? false)
    <a wire:key="is3liked-{{$post->id}}" title="you like this post" class="">
        <x-microblog.images.unlike-icon class="fill-red-300"/>
    </a>
    @else
    <a wire:key="is4liked-{{$post->id}}" wire:click="dislike" title="dislike" class="ml-2 cursor-pointer">
...
@else
        ...
        ({{$post->usersThatLike()->count()}})
    login to like
        <a href="#" title="login to dislike" class="pointer-events-none">
            <x-microblog.images.unlike-icon class="fill-red-300"/>
        </a>
        ({{$post->usersThatDislike()->count()}})
    login to dislike
